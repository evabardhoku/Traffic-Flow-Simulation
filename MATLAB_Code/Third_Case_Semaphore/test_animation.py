...
try:
    from First_case_Alinea_OnRamp.alinea_control import alinea, compute_density, r_prev, congestion_at_merge
    from traffic_demand import get_mainline_spawn_chance, format_time
    from traffic_demand import TOTAL_FRAMES
    from traffic_demand import get_ramp_spawn_chance
    from First_case_Alinea_OnRamp.sensors import Sensor
    from First_case_Alinea_OnRamp.metanet_analysis import handle_sensor_data, plot_global_analysis
    ...
    TOTAL_FRAMES = 3600  # Default total frames if not imported
...
# --- SEMAPHORE CONTROL VARIABLES ---
# This will hold the current state of the semaphore.
# 0: Controlled by ALINEA, 1: Manually Blocked, 2: Manually Allowed
semaphore_manual_override = 0
current_semaphore_color = "gray" # Default neutral color
...
def enforce_safe_distance(min_gap=0.2):  # Increased min_gap for better visual spacing
    ...
    for i in range(len(ramp_cars) - 1):
        ...
        if gap < min_gap * 0.7:  # Slightly tighter gap for ramp
            ...
            # Ramp cars also get a minimum speed, but their interaction with mainline is primary
    ...
    for car in cars:
        ...
        if car["type"] == "truck" or car["lane"] == "ramp":  # Trucks and ramp cars don't overtake
            continue
...
semaphore_circle = None # Global variable to hold the semaphore circle object

def draw_static_road(ax):
    global semaphore_circle
    road_y = 0
    lane_height = 0.3
    ...
    # Draw the ramp with a more integrated look using Polygon
    ramp_start_cell = 1.3  # Cell coordinate where ramp starts
    ramp_end_cell = 2.9  # Cell coordinate where ramp ends at mainline (visually under)

    ramp_y_start = -0.35  # Y-coordinate at ramp start
    # The crucial part: the merge point Y-coordinate for the ramp polygon should be *below* the main road.
    # The main road's bottom lane is at y=0.05 (for lane 1). So ramp must be < 0.05 here.
    ramp_y_merge_under = -0.05  # Y-position where ramp visually goes "under" the road

    # Convert cell coordinates to display coordinates
    visual_ramp_start_x = ramp_start_cell * DISPLAY_CELL_VISUAL_WIDTH
    visual_ramp_end_x = ramp_end_cell * DISPLAY_CELL_VISUAL_WIDTH

    # Define polygon points for a smoother, wider ramp
    # Bottom-left, Top-left, Top-right, Bottom-right
    ramp_poly_points = [
        (visual_ramp_start_x, ramp_y_start),
        (visual_ramp_start_x, ramp_y_start + 0.25),
        (visual_ramp_end_x, ramp_y_merge_under + 0.2),  # This point needs to be higher to define the top of the ramp
        (visual_ramp_end_x, ramp_y_merge_under),  # This point defines where the bottom of the ramp goes under
    ]
    ramp_polygon = Polygon(ramp_poly_points, closed=True, color='sienna', alpha=0.7, edgecolor='none',
                           zorder=1)  # Ramp behind road
    ax.add_patch(ramp_polygon)

    ramp_text_obj = ax.text((ramp_start_cell + (ramp_end_cell - ramp_start_cell) / 2) * DISPLAY_CELL_VISUAL_WIDTH,
                            ramp_y_start - 0.1, 'On-Ramp', ha='center', va='bottom', fontsize=9, color='white',
                            weight='bold', zorder=1)
    ramp_text_obj._static_label = True
    ...
    # --- Draw Semaphore ---
    semaphore_x = (MERGE_ZONE_START_X - 0.5) * DISPLAY_CELL_VISUAL_WIDTH
    semaphore_y = ramp_y_start + 0.4 # Position it above the ramp entrance
    semaphore_circle = Circle((semaphore_x, semaphore_y), 0.1, color=current_semaphore_color, zorder=6)
    ax.add_patch(semaphore_circle)
    semaphore_circle._static_label = True # Mark as static so it's not removed by general text clearing

    semaphore_text = ax.text(semaphore_x, semaphore_y + 0.15, 'Ramp Meter', ha='center', va='bottom', fontsize=9, color='black', weight='bold', zorder=6)
    semaphore_text._static_label = True
...
def update(frame):
    global r_prev, last_frame_time, fps_rolling_avg, current_semaphore_color
    ...
    alinea_allow = r_prev > 0.5 # ALINEA's decision

    if semaphore_manual_override == 1: # Manually Blocked
        current_semaphore_color = "red"
        # Ramp cars are blocked, so don't spawn
        ramp_chance = 0.0 # Effectively prevents new ramp car spawns
    elif semaphore_manual_override == 2: # Manually Allowed
        current_semaphore_color = "green"
        # Allow ramp cars to spawn regardless of ALINEA's r_prev
        if np.random.rand() < ramp_chance: # Use the original ramp_chance
            if not any(
                    car.get("lane") == "ramp" and MERGE_ZONE_START_X - 1.0 <= car["x"] < MERGE_ZONE_START_X - 0.5 for car in
                    cars):
                cars.append(spawn_ramp_car())
    else: # ALINEA Controlled (semaphore_manual_override == 0)
        if alinea_allow:
            current_semaphore_color = "green"
            if np.random.rand() < ramp_chance:
            ...
            current_semaphore_color = "red"
            ramp_chance = 0.0 # Block ramp car spawns

    # Update the visual semaphore color
    if semaphore_circle:
        semaphore_circle.set_color(current_semaphore_color)
    ...
    alinea_status_text = "ALINEA: ALLOW" if alinea_allow else "ALINEA: BLOCK"
    if semaphore_manual_override == 1:
        alinea_status_text = "Ramp Meter: MANUALLY BLOCKED"
    elif semaphore_manual_override == 2:
        alinea_status_text = "Ramp Meter: MANUALLY ALLOWED"

    ax.text(0, 1.7,  # Adjusted lower
            f"{alinea_status_text} | Merge Cell Density (Cell 2): {rho_merge_cell:.3f}",
...
# --- Semaphore Control Callbacks ---
def set_semaphore_alinea(event):
    global semaphore_manual_override
    semaphore_manual_override = 0
    print("Semaphore control set to ALINEA.")

def set_semaphore_block(event):
    global semaphore_manual_override
    semaphore_manual_override = 1
    print("Semaphore manually BLOCKED.")

def set_semaphore_allow(event):
    global semaphore_manual_override
    semaphore_manual_override = 2
    print("Semaphore manually ALLOWED.")
...
for text_obj in ax.texts:
    if 'Cell' in text_obj.get_text() or 'On-Ramp' in text_obj.get_text() or 'S' in text_obj.get_text() or 'Ramp Meter' in text_obj.get_text():
...
def stop_recording(event):
    ...
    try:
        from First_case_Alinea_OnRamp.metanet_analysis import handle_sensor_data, plot_global_analysis
        for sensor in sensors:
            print(f"DEBUG: Processing sensor {sensor.cell_index} data...")
            handle_sensor_data(
                sensor.cell_index,
                sensor.get_table(),
...
# --- Semaphore Control Buttons ---
ax_semaphore_alinea = plt.axes([0.45, 0.1, 0.15, 0.075]) # Positioned higher
ax_semaphore_block = plt.axes([0.45, 0.0, 0.07, 0.075]) # Lower left
ax_semaphore_allow = plt.axes([0.53, 0.0, 0.07, 0.075]) # Lower right

btn_semaphore_alinea = Button(ax_semaphore_alinea, 'ALINEA Control')
btn_semaphore_block = Button(ax_semaphore_block, 'Block')
btn_semaphore_allow = Button(ax_semaphore_allow, 'Allow')

btn_semaphore_alinea.on_clicked(set_semaphore_alinea)
btn_semaphore_block.on_clicked(set_semaphore_block)
btn_semaphore_allow.on_clicked(set_semaphore_allow)
